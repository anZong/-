...
module.exports = class {
  constructor(page, opt) { // opt 字段有：width, height, 分别代表canvas的宽高，px为单位，
    ...
    this.page = page;
    this.width = opt && opt.width || wx.getSystemInfoSync().screenWidth*0.8;
    this.height = opt && opt.height || this.width;
  ...
  createCircle() {// 创建解锁点的坐标，根据canvas的大小来平均分配半径
    ...
    var count = 0;
    this.r = this.width / (2*(2*n-1));// 公式计算
    this.lastPoint = [];
    this.arr = [];
    this.restPoint = [];
    var r = this.r;
  ...
  getPosition(e) { // 获取touch点相对于canvas的坐标
    var po = {
      x: e.touches[0] && e.touches[0].x,
      y: e.touches[0] && e.touches[0].y
    };
    return po;
  }
